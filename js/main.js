...
function init(){
  ...
  document.getElementById('btnCentrar').onclick = () => {
    controls.target.set(0,1,0); camera.position.set(0,1.6,4);
  };
  ...
  // Defaults
  cargarEscenario('./models/gltf/vr_art_gallery_room.glb');   // también puede ser .fbx
...
function fitSceneToCamera(root){
  ...
  camera.position.copy(center).add(new THREE.Vector3(0, maxDim*0.5, fitDist*1.2));
}
